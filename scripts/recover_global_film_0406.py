...
def _manifest_ready_for_upload(manifest_path: Path, serial: int) -> bool:
    try:
    ...
    if not isinstance(channel_payload, dict):
        return False
    if bool(channel_payload.get("_metadata_failed")):
        return False
    title = str(channel_payload.get("title") or "").strip()
    return bool(title)
...
async def main() -> None:
    ...
    for row in snapshot.get("tasks", []):
        ...
        if status == "existing_done":
            ...
            if ready_manifest.exists() and _manifest_ready_for_upload(ready_manifest, task.serial):
