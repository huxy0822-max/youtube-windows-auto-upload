...
from browser_api import list_browser_envs, set_runtime_provider, get_runtime_provider, probe_browser_providers, open_browser_env
...
class DashboardApp(ctk.CTk):
    ...
    def _build_variables(self) -> None:
        ...
        self.visual_spectrum_asset_var = ctk.StringVar(
            value=str(state.get("visual_spectrum_asset", visual_cfg.get("spectrum_asset", "code")) or "code")
        )
        ...
        self.visual_sticker_var = ctk.StringVar(
            value=str(state.get("visual_sticker", visual_cfg.get("sticker", "none")) or "none")
        )
        sticker_count_min, sticker_count_max = _split_range_value(
            state.get("visual_sticker_count", visual_cfg.get("sticker_count", "2,4")),
            2,
            4,
        )
        self.visual_sticker_count_min_var = ctk.StringVar(value=sticker_count_min)
        self.visual_sticker_count_max_var = ctk.StringVar(value=sticker_count_max)
        sticker_opacity_min, sticker_opacity_max = _split_range_value(
            state.get("visual_sticker_opacity", visual_cfg.get("sticker_opacity", "0.35,0.55")),
            0.35,
            0.55,
        )
        self.visual_sticker_opacity_min_var = ctk.StringVar(value=sticker_opacity_min)
        self.visual_sticker_opacity_max_var = ctk.StringVar(value=sticker_opacity_max)
    ...
    def _collect_visual_settings(self) -> dict[str, Any]:
        return {
            ...
            "spectrum_asset": self.visual_spectrum_asset_var.get().strip() or "code",
            "reactive_spectrum_enabled": bool(self.visual_reactive_spectrum_var.get()),
            "reactive_spectrum_preset": str(self.visual_reactive_spectrum_preset_var.get() or "random").strip()
            or "random",
            ...
            "sticker": self.visual_sticker_var.get().strip() or "none",
            "sticker_count": _compose_range_value(
                self.visual_sticker_count_min_var.get(),
                self.visual_sticker_count_max_var.get(),
                "2,4",
            ),
            "sticker_opacity": _compose_range_value(
                self.visual_sticker_opacity_min_var.get(),
                self.visual_sticker_opacity_max_var.get(),
                "0.35,0.55",
            ),
    ...
    def _serialize_visual_preset(self, name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "spectrum_asset": str(settings.get("spectrum_asset") or "code"),
            "reactive_spectrum_enabled": bool(settings.get("reactive_spectrum_enabled", False)),
            "reactive_spectrum_preset": str(settings.get("reactive_spectrum_preset") or "random"),
            ...
            "sticker": str(settings.get("sticker") or "none"),
            "sticker_count": str(settings.get("sticker_count") or "2,4"),
            "sticker_opacity": str(settings.get("sticker_opacity") or "0.35,0.55"),
    ...
    def _visual_preset_to_settings(self, preset_name: str) -> dict[str, Any]:
        ...
        sticker_value = str(payload.get("sticker") or payload.get("sticker_file") or "none").strip() or "none"
        ...
        return {
            "preset": preset_name,
            "spectrum": True,
            "spectrum_asset": str(payload.get("spectrum_asset") or "code").strip() or "code",
            "reactive_spectrum_enabled": str(
                payload.get("reactive_spectrum_enabled") or ""
            ).strip().lower()
            in {"1", "true", "yes", "on"},
            "reactive_spectrum_preset": str(payload.get("reactive_spectrum_preset") or "random").strip()
            or "random",
            ...
            "sticker_count": str(payload.get("sticker_count") or "2,4").strip() or "2,4",
            "sticker_opacity": str(payload.get("sticker_opacity") or "0.35,0.55").strip() or "0.35,0.55",
    ...
    def _apply_visual_settings_to_form(self, settings: dict[str, Any], *, preset_choice: str | None = None) -> None:
        ...
        self.visual_spectrum_asset_var.set(str(settings.get("spectrum_asset") or "code"))
        self.visual_reactive_spectrum_var.set(bool(settings.get("reactive_spectrum_enabled", False)))
        self.visual_reactive_spectrum_preset_var.set(str(settings.get("reactive_spectrum_preset") or "random"))
        ...
        self.visual_sticker_var.set(str(settings.get("sticker") or "none"))
        sticker_count_min, sticker_count_max = _split_range_value(settings.get("sticker_count", "2,4"), 2, 4)
        self.visual_sticker_count_min_var.set(sticker_count_min)
        self.visual_sticker_count_max_var.set(sticker_count_max)
        sticker_opacity_min, sticker_opacity_max = _split_range_value(settings.get("sticker_opacity", "0.35,0.55"), 0.35, 0.55)
        self.visual_sticker_opacity_min_var.set(sticker_opacity_min)
        self.visual_sticker_opacity_max_var.set(sticker_opacity_max)
        ...
        self._refresh_reactive_spectrum_controls()
    ...
    def _save_state(self) -> None:
        state = {
            ...
            "visual_spectrum_asset": self.visual_spectrum_asset_var.get(),
            "visual_reactive_spectrum_enabled": bool(self.visual_reactive_spectrum_var.get()),
            "visual_reactive_spectrum_preset": self.visual_reactive_spectrum_preset_var.get(),
            ...
            "visual_sticker": self.visual_sticker_var.get(),
            "visual_sticker_count": _compose_range_value(
                self.visual_sticker_count_min_var.get(),
                self.visual_sticker_count_max_var.get(),
                "2,4",
            ),
            "visual_sticker_count_min": self.visual_sticker_count_min_var.get(),
            "visual_sticker_count_max": self.visual_sticker_count_max_var.get(),
            "visual_sticker_opacity": _compose_range_value(
                self.visual_sticker_opacity_min_var.get(),
                self.visual_sticker_opacity_max_var.get(),
                "0.35,0.55",
            ),
            "visual_sticker_opacity_min": self.visual_sticker_opacity_min_var.get(),
            "visual_sticker_opacity_max": self.visual_sticker_opacity_max_var.get(),
    ...
    def _current_upload_defaults_model(self) -> UploadDefaults:
        ...
        return UploadDefaults(
            ...
            notify_subscribers="yes" if bool(self.default_notify_var.get()) else "no",
...
def _patched_update_run_status_from_log(self: DashboardApp, message: str) -> None:
    ...
    if text.startswith("[任务]"):
        ...
        if progress_match:
            self._run_current_ratio = max(0.0, min(1.0, int(progress_match.group(1)) / 100.0))
    elif text.startswith("[Metadata]") or text.startswith("[并行文案]") or text.startswith("[文案]"):
        self._run_phase = "metadata"
        metadata_match = re.search(r"\]\s+([^/]+)/([0-9]+(?:\[[0-9]+/[0-9]+\])?)", text)
        if metadata_match:
            self._run_current_item = f"{metadata_match.group(1)} / 窗口 {metadata_match.group(2)}"
            self._run_current_ratio = 0.0
    elif text.startswith("[thumb]"):
        self._run_phase = "thumbnail"
    elif text.startswith("[清单]"):
...
def _patched_build_variables_v2(self: DashboardApp) -> None:
    ...
    self.visual_reactive_spectrum_var = ctk.BooleanVar(
        value=bool(
            state.get(
                "visual_reactive_spectrum_enabled",
                visual_cfg.get("reactive_spectrum_enabled", False),
            )
        )
    )
    self.visual_reactive_spectrum_preset_var = ctk.StringVar(
        value=str(
            state.get(
                "visual_reactive_spectrum_preset",
                visual_cfg.get("reactive_spectrum_preset", "random"),
            )
            or "random"
        ).strip()
        or "random"
...
def _patched_bind_variable_events_v2(self: DashboardApp) -> None:
    ...
    self.visual_reactive_spectrum_var.trace_add("write", lambda *_: self._refresh_reactive_spectrum_controls())
    for variable in (
        self._step_generate_var,
        self._step_render_var,
        self._step_upload_var,
        self.queue_videos_per_window_var,
        self.visual_reactive_spectrum_var,
        self.visual_reactive_spectrum_preset_var,
    ):
...
def _patched_build_visual_tab_v2(self: DashboardApp) -> None:
    ...
    self._remember_visual_widgets("spectrum_asset", _dashboard_option_row(basic, 5, "频谱素材", self.visual_spectrum_asset_var, values=list_spectrum_assets()))
    self._remember_visual_widgets("style", _dashboard_option_row(basic, 6, "代码频谱样式", self.visual_style_var, values=_with_random(list_effects())))
    self._remember_visual_widgets("spectrum_y", _dashboard_option_row(basic, 7, "频谱 Y", self.visual_spectrum_y_var))
    self._remember_visual_widgets("spectrum_x", _dashboard_option_row(basic, 8, "频谱 X (-1=居中)", self.visual_spectrum_x_var))
    self._remember_visual_widgets("spectrum_w", *_dashboard_range_row(basic, 9, "频谱宽度", self.visual_spectrum_w_min_var, self.visual_spectrum_w_max_var))
    ...
    reactive_row = ctk.CTkFrame(pulse, fg_color="transparent")
    reactive_row.grid(row=4, column=0, columnspan=4, sticky="ew", padx=16, pady=(0, 8))
    reactive_row.grid_columnconfigure(2, weight=1)
    reactive_label = ctk.CTkLabel(reactive_row, text="真实音频驱动频谱")
    reactive_label.grid(row=0, column=0, sticky="w", padx=(0, 12), pady=0)
    reactive_check = ctk.CTkCheckBox(
        reactive_row,
        text="启用",
        variable=self.visual_reactive_spectrum_var,
        font=_dashboard_theme_font(),
    )
    reactive_check.grid(row=0, column=1, sticky="w", padx=(0, 12), pady=0)
    self.visual_reactive_spectrum_preset_menu = ctk.CTkOptionMenu(
        reactive_row,
        variable=self.visual_reactive_spectrum_preset_var,
        values=_with_random(list_reactive_spectrum_presets()),
        font=_dashboard_theme_font(),
        dropdown_font=_dashboard_theme_font(),
    )
    self.visual_reactive_spectrum_preset_menu.grid(row=0, column=2, sticky="ew", padx=(0, 12), pady=0)
    reactive_hint = ctk.CTkLabel(
        reactive_row,
        text="勾选后改走真实音频驱动频谱；不勾选时使用高质量 loop 频谱素材。",
        text_color="#b8c3cf",
        anchor="w",
    )
    reactive_hint.grid(row=0, column=3, sticky="w", pady=0)
    self._remember_visual_widgets("reactive_spectrum", reactive_check, self.visual_reactive_spectrum_preset_menu)

    overlay = ctk.CTkFrame(tab)
    overlay.grid(row=4, column=0, sticky="ew", padx=8, pady=8)
    for column in range(4):
        overlay.grid_columnconfigure(column, weight=1)
    ctk.CTkLabel(overlay, text="粒子 / 贴纸 / 叠字", font=ctk.CTkFont(size=22, weight="bold")).grid(
        row=0, column=0, columnspan=4, sticky="w", padx=16, pady=(14, 12)
    )
    ...
    self._remember_visual_widgets("sticker", _dashboard_option_row(overlay, 4, "贴纸素材", self.visual_sticker_var, values=list_sticker_effects()))
    self._remember_visual_widgets("sticker_count", *_dashboard_range_row(overlay, 5, "贴纸数量", self.visual_sticker_count_min_var, self.visual_sticker_count_max_var))
    self._remember_visual_widgets("sticker_opacity", *_dashboard_range_row(overlay, 6, "贴纸透明度", self.visual_sticker_opacity_min_var, self.visual_sticker_opacity_max_var))
    self._remember_visual_widgets("text", _dashboard_option_row(overlay, 7, "叠字内容", self.visual_text_var))
    self._remember_visual_widgets("text_font", _dashboard_option_row(overlay, 8, "字体", self.visual_text_font_var, values=_with_random(list_font_names())))
    self._remember_visual_widgets("text_pos", _dashboard_option_row(overlay, 9, "文字位置", self.visual_text_pos_var, values=_with_random(list_text_positions())))
    self._remember_visual_widgets("text_size", *_dashboard_range_row(overlay, 10, "文字大小", self.visual_text_size_min_var, self.visual_text_size_max_var))
    self._remember_visual_widgets("text_style", _dashboard_option_row(overlay, 11, "文字样式", self.visual_text_style_var, values=_with_random(list_text_styles())))
...
def _patched_refresh_reactive_spectrum_controls_v1(self: DashboardApp) -> None:
    menu = getattr(self, "visual_reactive_spectrum_preset_menu", None)
    if menu is None:
        return
    state = "normal" if bool(self.visual_reactive_spectrum_var.get()) else "disabled"
    try:
...
def _patched_window_default_values_v3(self: DashboardApp, job: GroupJob, serial: int) -> dict[str, str]:
    ...
    ypp = str((override.ypp if override and override.ypp else _yes_no_from_bool(bool(info.is_ypp))) or "no").strip() or "no"
    notify = str((override.notify_subscribers if override and override.notify_subscribers else (defaults.notify_subscribers or "no")) or "no").strip() or "no"
    return {
        "ypp": ypp,
        "visibility": visibility,
        "category": category,
...
def _patched_open_window_override_dialog_v3(self: DashboardApp, index: int) -> None:
    ...
    for column in range(10):
        table.grid_columnconfigure(column, weight=1)
    headers = ["窗口", "YPP", "可见性", "分类", "儿童内容", "AI 内容", "通知订阅", "定时发布", "日期", "时间"]
    for column, title in enumerate(headers):
        ctk.CTkLabel(table, text=title, font=ctk.CTkFont(weight="bold")).grid(row=0, column=column, sticky="w", padx=6, pady=(4, 8))
    ...
    for row_index, serial in enumerate(job.window_serials, start=1):
        ...
        vars_for_row = {
            ...
            "kids_content": ctk.StringVar(value=defaults["kids_content"]),
            "ai_content": ctk.StringVar(value=defaults["ai_content"]),
            "notify_subscribers": ctk.StringVar(value=defaults.get("notify_subscribers", "no")),
    ...
    def reset_to_defaults() -> None:
        for serial, context in row_contexts.items():
            ...
            context["vars"]["notify_subscribers"].set(defaults.get("notify_subscribers", "no"))
    ...
    def save_dialog() -> None:
        ...
        for serial, context in row_contexts.items():
            ...
            override = WindowOverride(
                ...
                kids_content="" if vars_for_row["kids_content"].get() == defaults["kids_content"] else vars_for_row["kids_content"].get(),
                ai_content="" if vars_for_row["ai_content"].get() == defaults["ai_content"] else vars_for_row["ai_content"].get(),
                notify_subscribers="" if vars_for_row["notify_subscribers"].get() == defaults.get("notify_subscribers", "no") else vars_for_row["notify_subscribers"].get(),
...
def _patched_build_window_tasks_from_job_v3(self: DashboardApp, job: GroupJob) -> list[WindowTask]:
    ...
    if job.upload_defaults:
        upload_defaults = UploadDefaults.from_dict(job.upload_defaults.to_dict())
    default_notify = str(upload_defaults.notify_subscribers or "no").strip().lower()
    tasks: list[WindowTask] = []
    seen_serials: set[int] = set()
    ...
    for raw_serial in job.window_serials:
        ...
        kids_value = override.kids_content if override and override.kids_content else _yes_no_from_bool(upload_defaults.is_for_kids)
        ai_value = override.ai_content if override and override.ai_content else (upload_defaults.ai_content or upload_defaults.altered_content or "yes")
        notify_value = override.notify_subscribers if override and override.notify_subscribers else default_notify
...
def _patched_start_real_flow_v3(self: DashboardApp) -> None:
    ...
    def maybe_preheat_browser(serial_value: int, group_tag: str) -> None:
        serial = int(serial_value or 0)
        if serial <= 0 or serial in preheated_serials or self._cancel_requested:
            return
        if not bool(tracking_plan.modules.upload):
            return
        provider_choice = str(getattr(self, "browser_provider_var", None).get() if getattr(self, "browser_provider_var", None) else "auto").strip().lower() or "auto"
        try:
            result = open_browser_env(
                str(serial),
                provider=provider_choice if provider_choice != "auto" else None,
            )
            preheated_serials.add(serial)
            data = result.get("data", {}) if isinstance(result, dict) else {}
            debug_port = str(data.get("http") or data.get("debug_port") or "").strip()
            self._log(
                f"[预热] {group_tag}/{serial} 浏览器已预热"
                + (f" | port={debug_port}" if debug_port else "")
            )
        except Exception as exc:
            self._log(f"[预热] {group_tag}/{serial} 浏览器预热失败: {exc}")
    ...
    def handle_queue_progress(event: dict[str, Any]) -> None:
        ...
        if event_type == "prepare_finished":
            ...
            self._run_current_ratio = 0.0
            return
        if event_type == "metadata_ready":
            title_preview = str(event.get("title") or "").strip()
            cover_count = int(event.get("cover_count") or 0)
            if group_tag and serial_value:
                self._mark_run_stage(
                    group_tag,
                    serial_value,
                    "metadata",
                    "success",
                    f"文案/缩略图已完成 | covers={cover_count}",
                    slot_index=slot_index,
                    total_slots=total_slots,
                )
            slot_suffix = f" [{slot_index}/{total_slots}]" if total_slots > 1 else ""
            self._run_phase = "生成标题/简介/标签/缩略图"
            self._run_current_item = f"{group_tag} / 窗口 {serial_value}{slot_suffix} | {title_preview[:42] or 'metadata ready'}"
            self._run_current_ratio = 0.0
            self._log(
                f"[Metadata] {group_tag}/{serial_value}{slot_suffix}: ready | "
                f"API={str(event.get('api_preset_name') or '').strip()} | "
                f"模板={str(event.get('content_template_name') or '').strip()} | "
                f"covers={cover_count}"
            )
            maybe_preheat_browser(serial_value, group_tag)
            return
        if event_type == "window_started":
            if label and not bool(event.get("has_prepare_step", True)):
                self._run_progress_step_done(label, "render")
...
DashboardApp._refresh_reactive_spectrum_controls = _patched_refresh_reactive_spectrum_controls_v1
